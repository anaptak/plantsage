...
function App() {
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      requestAnimationFrame(() => {
        ...
        console.log('Image Height (px)    :', imageHeight);
        console.log('Desired Bottom Y (px):', desiredBottomY);  
        console.log('Screen Height (px)  :', screenHeight); 
        console.log('Initial Offset (px) :', initialOffsetPx);  
        console.log('Hero Height (px)    :', heroHeight); 
        console.log('Final Offset (px)   :', finalOffsetPx);  
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      console.log('Screen Height (px)  :', screenHeight);
      console.log('Initial Offset (px) :', initialOffsetPx);
      console.log('Old Height (px)     :', oldHeight);
      console.log('New Height (px)     :', newHeight);
      console.log('Delta Center (px)   :', deltaCenter);
      console.log('Final Offset (px)   :', finalOffsetPx);
      console.log('Offset Y (%)         :', finalOffsetPercent);
      setBgPositionY(`center ${finalOffsetPercent}%`);
    };
      ...
      <div className="overflow-x-auto w-full px-4 py-12">
        {plantTitle && (
          <div className="mb-8 text-center max-w-3xl mx-auto">
            <h2 className="text-3xl font-bold text-white font-['Playfair_Display']">{plantTitle}</h2>
            <p className="mt-3 text-base text-white leading-relaxed">{plantDescription}</p>
          </div>
        )}
